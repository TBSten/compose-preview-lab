...
例えば、`@CollectPreviews` で Preview を自動収集する代わりに、手動で PreviewLabPreview のリストを定義して PreviewLabGallery に渡すことも可能です。
...
- **Compiler Plugin** は プロジェクト内の `@Preview` を収集し、`@CollectPreviews` が付与されたプロパティに Preview リストを注入します。
