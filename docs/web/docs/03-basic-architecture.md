...
- [**Build Tooling**](#build-tooling) ... Gradle Plugin, Compiler Plugin をセットアップすることで Preview を収集するなどの機能を提供します。
...
例えば、`collectModulePreviews()` で Preview を自動収集する代わりに、手動で PreviewLabPreview のリストを定義して PreviewLabGallery に渡すことも可能です。

## Build Tooling

[インストール](./install/cmp) に従うと Gradle Plugin と Compiler Plugin がセットアップされます。

それぞれ以下のような機能があります。

- **Gradle Plugin** には Compiler Plugin のセットアップを簡素化するための DSL と [FeaturedFile](./guides/featured-files) サポートのためのコード自動生成ロジックが含まれます。
- **Compiler Plugin** は プロジェクト内の `@Preview` を収集し、`collectModulePreviews()` / `collectAllModulePreviews()` で宣言したプロパティに Preview リストを注入します。
