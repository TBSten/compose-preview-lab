...
- previewList 引数には `@CollectPreviews` で収集したプレビューリスト、または手動で設定した PreviewLabPreview のインスタンスを指定してください。
...
`@CollectPreviews` で収集したプレビューリストを渡してください。ここに渡されたプレビューが一覧表示されます。または 自分で PreviewLabPreview のインスタンスを作成して渡すこともできます。Preview の収集については [Collect Preview](./collect-preview) を参照してください。
