...
public interface CompatContext {
     ...
     * `.class` files, KLIB writes them into the `.klib`. Cross-module *discovery* of
     * those declarations from the IR side has separate platform-version constraints —
     ...
     * discovery is **safe to use on KLIB targets** (JS / Wasm / iOS / native) on the
     * current Kotlin compiler.
     *
     ...
     * top-level declarations correctly at FIR time but leak stale entries through the
     * KLIB incremental-compile cache, so a downstream consumer can pick up the wrong
     * hint and `collectAllModulePreviews()` returns a broken aggregation.
     *
