...
2. **設計・アーキテクチャ**
    ...
    - モジュールが不要な依存関係を持ってないか？
        - ui component が必要な場合, material の利用は最低限にし、projects.ui を使っているか

3. **動作検証・テスト**
    - `jvmTest` (トップ, integrationTest 両方), `buildProductionPreviewLabGallery` でそれぞれビルドできるか？
    - 新機能のテスト有無、エッジケースのカバー
    - 適切な Example Based Test / Property Based Test / IntegrationTest が用意されているか

4. **ドキュメント**
    - 対応するドキュメントが適切・正確・わかりやすく更新されているか
