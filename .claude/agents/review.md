...
1. 引数（`$1`）を確認:
    - PR番号（`#123` or `123`）→ PR の base ブランチ, head ブランチ を取得してそれの差分をレビュー
    - ブランチ名 → ブランチ - main をレビュー。Github CLI で該当 PR がないかチェックして、該当PR があればそれに基づいてレビューする。
    - 指定なし → 現在のブランチ - main をレビュー。Github CLI で該当 PR がないかチェックして、該当PR があればそれに基づいてレビューする。
2. 現在の状態を確認:
   ```bash
   git branch
   git status
