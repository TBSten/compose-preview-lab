...
#### 4.3 PRタイトルの作成
- **タイトル先頭に内容を表す絵文字 prefix を 1 つ付ける**（例: `🗑️ 不要なコード削除`）
- コミットメッセージのスタイルを参考に作成
- プレフィックスを含める: `feat:`, `fix:`, `docs:`, `refactor:`, `test:`, `chore:`
  - 絵文字と Conventional Commits 風 prefix を併用する場合は `絵文字 + プレフィックス + 内容` の順（例: `✨ feat: add user login`）
- 50文字以内で簡潔に（絵文字を含めて）
...
3. `run_terminal_cmd` で以下のコマンドを実行:
   ```bash
   gh pr create --title "<絵文字> <PRタイトル>" --body "$(cat <<'EOF'
   <PR本文>
   EOF
   )" --draft
   ```
   - `<絵文字>` は Step 4.3「絵文字 prefix の選択」で決めたものを必ず先頭に置く
